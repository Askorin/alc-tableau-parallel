...
class SerialReasoner : public Reasoner {
  private:
        ...
        for (const Concept* c : arena.getLabels(node_idx)) {
            if (c->type == ConceptType::NEGATION) {
                ...
                if (arena.hasLabel(node_idx, neg->inner)) {
                    const_cast<ThreadLocalArena&>(arena).getNode(node_idx).isClashed = true;
                    return true;
                }
            }
        }
    ...
    bool isSubset(size_t sub_idx, size_t super_idx, const ThreadLocalArena& arena) {
    ...
    bool isBlocked(size_t node_idx, const std::vector<size_t>& ancestors, const ThreadLocalArena& arena) {
    ...
    bool saturatePropositional(size_t node_idx, ThreadLocalArena& arena) {
        ...
        while (changed) {
            ...
            std::vector<const Concept*> snapshot(current_labels.begin(), current_labels.end());

            for (const Concept* c : snapshot) {
                if (c->type == ConceptType::CONJUNCTION) {
                    const auto* conj = static_cast<const ConjunctionConcept*>(c);
            ...
            if (changed && checkForClash(node_idx, arena)) return false;
        }
        return true;
    }

    std::vector<size_t> generateFrontier(size_t node_idx, ThreadLocalArena& arena) {
        auto current_labels = arena.getLabels(node_idx);
        std::vector<const Concept*> snapshot(current_labels.begin(), current_labels.end());
        ...
        for (const Concept* c : snapshot) {
            if (c->type == ConceptType::EXISTENTIAL) {
                ...
                for (const Edge& edge : arena.getEdges(node_idx)) {
                    if (edge.role == ex->role && arena.hasLabel(edge.child_idx, ex->inner)) {
        ...
        for (const Concept* c : snapshot) {
            if (c->type == ConceptType::UNIVERSAL) {
                const auto* univ = static_cast<const RoleRestrictionConcept*>(c);
                
                auto current_edges = arena.getEdges(node_idx);
                std::vector<Edge> edge_snapshot(current_edges.begin(), current_edges.end());
                
                for (const Edge& edge : edge_snapshot) {
                    if (edge.role == univ->role && !arena.hasLabel(edge.child_idx, univ->inner)) {
    ...
    bool hasPendingDisjunctions(size_t node_idx, const ThreadLocalArena& arena) {
        for (const Concept* c : arena.getLabels(node_idx)) {
    ...
    bool resolveDisjunctions(size_t node_idx, ThreadLocalArena& arena, ArenaPool& pool, std::vector<size_t>& ancestors) {
        auto current_labels = arena.getLabels(node_idx);
        std::vector<const Concept*> labels_snapshot(current_labels.begin(), current_labels.end());
        ...
        for (const Concept* c : labels_snapshot) {
            if (c->type == ConceptType::DISJUNCTION) {
                ...
                if (!arena.hasLabel(node_idx, disj->left) && 
                    ...
                    if (processNode(node_idx, copyLeft, pool, ancestors)) {
                        return true;
                    }
                    ...
                    return processNode(node_idx, copyRight, pool, ancestors);
                }
            }
        }
        return false;
    ...
    bool processNode(size_t node_idx, ThreadLocalArena& arena, ArenaPool& pool, std::vector<size_t>& ancestors) {
        ...
        if (hasPendingDisjunctions(node_idx, arena)) {
            return resolveDisjunctions(node_idx, arena, pool, ancestors); 
        }

        std::vector<size_t> frontier = generateFrontier(node_idx, arena);
        if (frontier.empty()) return true; // Leaf node

        ancestors.push_back(node_idx);

        for (size_t child_idx : frontier) {
            if (!processNode(child_idx, arena, pool, ancestors)) {
                ancestors.pop_back();
                return false; 
            }
        }
  ...
  public:
    ...
    bool isSatisfiable(const Concept* rootConcept) override {
        ...
        return processNode(root_idx, arena, pool, root_ancestors);
    }
};
